...
func TestProtoComms_PollNdf(t *testing.T) {
	...
	if err == nil {
		t.Logf("GetHost should have failed but it didnt't: %+v", err)
		t.Fail()
	}
...
func (m mockProtoComms) PollNdf(currentDef *ndf.NetworkDefinition) (*ndf.NetworkDefinition, error) {
	realComms := ProtoComms{}
	return realComms.PollNdf(currentDef)
}

// Mock the Get host function to return
func (m mockManager) GetHost(hostId string) (*Host, bool) {
	return nil, GetHostErrBool
}

func (m mockProtoComms) RequestNdf(host *Host, message *pb.NDFHash) (*pb.NDF, error) {
